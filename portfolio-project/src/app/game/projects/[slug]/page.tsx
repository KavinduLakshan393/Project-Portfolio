...
export async function generateMetadata({
  ...
  return {
    title: `${project.title} | Game Project`,
    description: project.summary,
  };
}

export default async function GameProjectDetails({ params }: ProjectPageProps) {
  const { slug } = await params;
  const project = gameProjects.find((item) => item.slug === slug);
  ...
  return (
    <main className="theme-game min-h-screen bg-[#0e0e0e] px-6 py-24 text-[#e5e2e1]">
